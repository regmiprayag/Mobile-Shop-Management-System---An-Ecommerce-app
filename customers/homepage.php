            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    if (isset($_SESSION['email'])) {
                        $_SESSION['product_model'] = $row['model'];
                        ...
                        echo '<a href="loginCustomer.php"><button onclick="myFunction()" class="w-full text-gray-300 bg-blue-600 px-6 p-2 rounded-lg hover:bg-blue-800">Add To Cart</button></a>';
                    }
                    echo '</div>';
                    echo '</div>';
                }
